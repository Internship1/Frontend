...
.controller('studRegServiceCtrl', function($scope, $http) {
	...
	$scope.postRegister = function (full_name, email, gender, username, password) {
		...
		//call service
		$http.post('http://192.168.0.9:8080/api/registerStudent', JSON.stringify(data)).then(function(response) {
			if (response.data)
	...
	$scope.postDetails = function (description, contact, address) {
		...
		var userEditProfileUrl = 'http://192.168.0.9:8080/api/usersProfile/store/#';
